            ...
            for algo_name in os.listdir(os.path.join(args.log_dir,env_dir)):
                ...
                for log_data_dir in os.listdir(algo_path):
                    ...
                    if args.key_word in log_data_path: 
                        ...
                        log_data.Reload()
                        algo_data_x.extend(smooth_step([data.step for data in log_data.scalars.Items(args.key_word)]))
                        algo_data_y.extend(smooth_value([data.value for data in log_data.scalars.Items(args.key_word)]))
                        env_num += 1
