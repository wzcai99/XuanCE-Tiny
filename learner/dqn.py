...
class DQN_Learner:
    ...
    def update(self,input_batch,action_batch,reward_batch,terminal_batch,next_input_batch):
        ...
        if self.iterations % self.update_frequency == 0:
            for ep,tp in zip(self.policy.eval_representation.parameters(),self.policy.target_representation.parameters()):
                tp.data.copy_(ep.data)
            for ep,tp in zip(self.policy.evalQ.parameters(),self.policy.targetQ.parameters()):
                tp.data.copy_(ep.data)
